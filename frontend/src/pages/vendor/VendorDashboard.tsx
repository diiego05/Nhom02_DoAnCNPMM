...
const CustomSelect = ({ label, value, options, onChange, placeholder = "-- Chọn --" }: CustomSelectProps) => {
  ...
  return (
    <div className="space-y-2 relative" ref={selectRef}>
      ...
      >
        <span className="text-xs font-bold">{selectedOption ? selectedOption.label : placeholder}</span>
        <ChevronDown size={20} className={`transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} style={{ strokeWidth: 3 }} />
      </button>
      {isOpen && (
...
const VendorDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === "overview") {
      vendorService.getShopStatistics()
        .then(res => {
          if (res && res.data) {
            setStats(res.data);
          }
    ...
    try {
      ...
      if (hasVariants) {
        if (productVariants.length === 0) {
          alert("Vui lòng thêm ít nhất một biến thể hoặc tắt chế độ đa biến thể!");
          return;
        }
        finalVariants = productVariants.map((v, i) => ({
          ...
          sale_price: v.sale_price ? parseFloat(v.sale_price.toString()) : undefined,
      ...
      const payload: ProductData = {
        ...
        brand_id: productForm.brand_id ? parseInt(productForm.brand_id) : undefined,
        ...
        images: productForm.image_url ? [{ image_url: productForm.image_url }] : []
      };
  ...
  const startEditProduct = (prod: any) => {
    ...
    if (prod.variants && prod.variants.length > 0) {
      ...
      const isDefault = prod.variants.length === 1 &&
        prod.variants[0].size === "Free Size" &&
        (prod.variants[0].color === "Default" || prod.variants[0].color === "Mặc định");
  ...
  const generateBulkVariants = () => {
    ...
    const sizes = bulkSizes ? bulkSizes.split(",").map(s => s.trim()).filter(Boolean) : ["Free Size"];
    const colors = bulkColors ? bulkColors.split(",").map(c => c.trim()).filter(Boolean) : ["Default"];

    const newVariants: any[] = [];
    sizes.forEach(size => {
      colors.forEach(color => {
        // Kiểm tra xem biến thể này đã tồn tại chưa để tránh trùng lặp
        const exists = productVariants.some(v => v.size.toLowerCase() === size.toLowerCase() && v.color.toLowerCase() === color.toLowerCase());
        if (!exists) {
          newVariants.push({
            size,
            color,
  ...
  const handlePrepareOrder = async (orderId: number) => {
    setConfirmModal({
      isOpen: true,
      title: "Chuẩn bị hàng",
      message: "Bạn có chắc chắn đã đóng gói xong và muốn chuyển trạng thái đơn hàng sang chuẩn bị hàng?",
      onConfirm: async () => {
        setConfirmModal(prev => ({ ...prev, isOpen: false }));
        try {
  ...
  const handleVoucherSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: VoucherData = {
        ...
        max_discount: voucherForm.max_discount ? parseFloat(voucherForm.max_discount.toString()) : undefined,
        usage_limit: voucherForm.usage_limit ? parseInt(voucherForm.usage_limit.toString()) : undefined,
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F4F4F0] flex items-center justify-center">
        <p className="text-xl font-black uppercase tracking-widest animate-pulse">Đang tải dữ liệu...</p>
      </div>
    );
  }
  ...
  const getRecentActivities = () => {
    ...
    orders.forEach((order: any) => {
      if (order.status === 'PENDING') {
        activities.push({
          ...
          time: order.created_at ? new Date(order.created_at).toLocaleTimeString("vi-VN", { hour: '2-digit', minute: '2-digit' }) : "Vừa xong",
        ...
        activities.push({
          ...
          time: order.created_at ? new Date(order.created_at).toLocaleDateString("vi-VN") : "Gần đây",
    ...
    reviews.forEach((review: any) => {
      activities.push({
        ...
        time: review.created_at ? new Date(review.created_at).toLocaleDateString("vi-VN") : "Gần đây",
    ...
    if (activities.length === 0) {
      return [
        { title: "Hệ thống hoạt động bình thường", time: "Vừa xong", icon: <CheckCircle2 size={14} className="text-green-500" /> },
        { title: "Đang chờ đơn hàng và tương tác đầu tiên", time: "Hôm nay", icon: <Clock size={14} className="text-orange-500" /> }
      ];
    }
      ...
      <main className="flex-grow flex flex-col h-screen overflow-y-auto relative">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-black/5 h-20 px-10 flex items-center justify-between sticky top-0 z-40 shadow-sm">
          ...
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-end">
              <span className="text-xs font-black uppercase text-gray-500">{shopInfo?.name}</span>
        ...
        <div className="p-10 max-w-7xl w-full mx-auto">
            ...
            <div className="space-y-10">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-gray-500 font-medium italic">Tổng hợp tình hình kinh doanh của shop trong ngày.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { label: "Doanh thu", value: `${stats.revenue.toLocaleString()}₫`, sub: "Hôm nay", icon: <Wallet className="text-green-500" /> },
                  { label: "Đơn hàng", value: stats.ordersCount.toString(), sub: "Hôm nay", icon: <ShoppingBag className="text-blue-500" /> },
                  { label: "Sản phẩm đang bán", value: stats.productsCount.toString(), sub: "Hoạt động", icon: <Package className="text-orange-500" /> },
                  { label: "Bình luận & Đánh giá", value: stats.commentsCount.toString(), sub: "Tất cả", icon: <MessageSquare className="text-red-500" /> }
                ].map((stat, i) => (
                ...
                <div className="lg:col-span-2 bg-white border-2 border-black rounded-[2rem] p-8 shadow-sm">
                  ...
                  <div className="h-64 flex items-end justify-between gap-3 px-4">
                    {stats.dailyRevenue.map((val, i) => {
                      const heightPercentage = Math.round((val / maxRevenue) * 100);
                      return (
                        <div key={i} className="flex-grow bg-black rounded-t-xl hover:bg-primary transition-all relative group cursor-pointer" style={{ height: `${Math.max(heightPercentage, 5)}%` }}>
                          <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap">{val.toLocaleString()}₫</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              {productView === "list" ? (
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      onClick={() => {
                        setEditingProduct(null);
                        setProductForm({
                          name: "", category_id: categories[0]?.id?.toString() || "", brand_id: "", description: "", price: 0, sale_price: "", gender: "UNISEX", material: "", stock_quantity: 10, image_url: ""
                        });
                        setHasVariants(false);
                        setProductVariants([]);
                        setBulkSizes("");
                  ...
                  <div className="bg-white border-2 border-black rounded-[2rem] overflow-hidden shadow-sm">
                    <div className="p-6 border-b-2 border-black/5 bg-gray-50/50 flex gap-4">
                      <div className="relative flex-grow">
                        <input type="text" placeholder="Tìm tên sản phẩm..." className="w-full bg-white border-2 border-black rounded-xl px-10 py-2.5 text-xs font-bold" />
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y-2 divide-black/5">
                          {products.length === 0 ? (
                            <tr>
                              <td colSpan={6} className="px-8 py-8 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">Không có sản phẩm nào</td>
                            </tr>
                          ) : (
                            products.map((prod, i) => (
                              <tr key={prod.id || i} className="hover:bg-gray-50 transition-colors group">
                                <td className="px-8 py-4">
                                  <div className="flex items-center gap-4">
                                    <div className="w-10 h-12 bg-gray-100 rounded-lg border border-black/5 overflow-hidden shrink-0">
                                      <img src={prod.images && prod.images[0] ? prod.images[0].image_url : "https://images.unsplash.com/photo-1591047139829-d91aecb6caea?q=80&w=100"} className="w-full h-full object-cover" />
                                    </div>
                                    <span className="text-xs font-black uppercase truncate max-w-[200px]">{prod.name}</span>
                                  </div>
                                </td>
                                <td className="px-8 py-4 text-xs font-mono font-bold text-gray-400">{prod.variants && prod.variants[0] ? prod.variants[0].sku : `PROD-00${prod.id}`}</td>
                                <td className="px-8 py-4 text-xs font-black">{prod.price.toLocaleString()}₫</td>
                                <td className="px-8 py-4 text-xs font-black text-primary">{prod.stock_quantity || (prod.variants && prod.variants.reduce((sum: number, v: any) => sum + v.stock_quantity, 0)) || 0}</td>
                                <td className="px-8 py-4">
                                  <span className={`text-[9px] font-black uppercase px-2 py-1 rounded border ${prod.status === 'ACTIVE' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                ...
                <div className="bg-white border-2 border-black rounded-[2.5rem] p-10 shadow-sm space-y-8 animate-in fade-in duration-200">
                  <div className="flex items-center gap-4 border-b-2 border-black/5 pb-6">
                    ...
                    <div>
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Quay lại danh sách</span>
                      <h2 className="text-2xl font-serif font-black uppercase tracking-tight">
                        {productView === "add" ? "Thêm sản phẩm mới" : "Cập nhật thông tin sản phẩm"}
                      </h2>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleProductSubmit} className="space-y-8">
                    ...
                    <div className="space-y-4">
                      <h3 className="text-sm font-black uppercase tracking-widest text-primary border-l-4 border-primary pl-2">1. Thông tin chung</h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Chất liệu sản phẩm</label>
                          <input
                            type="text"
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Ảnh sản phẩm</label>
                            ...
                            <div className="flex items-center gap-3">
                              <label className="flex-grow flex items-center justify-center border-2 border-dashed border-black rounded-xl py-3 px-4 bg-gray-50 hover:bg-gray-100/50 cursor-pointer transition-colors relative active:translate-y-[1px]">
                                ...
                                <span className="text-xs font-bold text-gray-600">
                                  {isUploadingProductImage ? "Đang tải ảnh lên..." : "Chọn ảnh từ thiết bị của bạn"}
                                </span>
                              </label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Mô tả chi tiết sản phẩm</label>
                        <textarea
                          rows={4}
                    ...
                    <div className="space-y-6 pt-4 border-t border-black/5">
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="text-sm font-black uppercase tracking-widest text-primary border-l-4 border-primary pl-2">2. Cấu hình phân loại & biến thể</h3>
                          <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">Bật chế độ này nếu sản phẩm của bạn có nhiều kích cỡ hoặc màu sắc khác nhau</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className={`text-xs font-black uppercase ${hasVariants ? 'text-primary' : 'text-gray-400'}`}>Có nhiều biến thể</span>
                      ...
                      {!hasVariants ? (
                        ...
                        <div className="bg-gray-50 border-2 border-black rounded-2xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-200">
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giá bán cơ bản (₫)</label>
                            <input
                              type="number"
                          ...
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giá khuyến mãi (₫ - Nếu có)</label>
                            <input
                              type="number"
                              className="w-full bg-white border-2 border-black rounded-xl px-4 py-3 font-bold focus:outline-none"
                          ...
                          <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Số lượng tồn kho chung</label>
                            <input
                              type="number"
                              className="w-full bg-white border-2 border-black rounded-xl px-4 py-3 font-bold focus:outline-none"
                        ...
                        <div className="space-y-8 animate-in fade-in duration-200">
                          ...
                          <div className="bg-[#FAF9F6] border-2 border-dashed border-black rounded-[2rem] p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white border-2 border-black rounded-2xl p-6 shadow-sm">
                              <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Kích thước (cách nhau bởi dấu phẩy)</label>
                                <input
                                  type="text"
                              ...
                              <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Màu sắc (cách nhau bởi dấu phẩy)</label>
                                <input
                                  type="text"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 bg-white border-2 border-black rounded-2xl p-6 shadow-sm items-end">
                              <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giá gốc chung (₫)</label>
                                <input
                                  type="number"
                              ...
                              <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giá khuyến mãi chung (₫)</label>
                                <input
                                  type="number"
                              ...
                              <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Số lượng tồn kho áp dụng chung</label>
                                <input
                                  type="number"
                          ...
                          <div className="space-y-4">
                            <div className="flex justify-between items-center">
                              <h4 className="text-xs font-black uppercase tracking-wider text-gray-500">Danh sách các biến thể đang có ({productVariants.length})</h4>
                              <button
                                type="button"
                                ...
                                <table className="w-full text-left">
                                  <thead>
                                    <tr className="border-b-2 border-black/5 text-[9px] font-black uppercase tracking-widest text-gray-400 bg-gray-50/50 sticky top-0 z-10">
                                      ...
                                      <th className="px-6 py-3">Màu sắc (Color)</th>
                                  ...
                                  <tbody className="divide-y divide-black/5">
                                    {productVariants.length === 0 ? (
                                      <tr>
                                        <td colSpan={7} className="px-6 py-8 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">
                                          Chưa có biến thể nào được tạo. Hãy sử dụng bảng tạo nhanh ở trên hoặc bấm thêm biến thể đơn lẻ!
                                        </td>
                                      </tr>
                                    ) : (
                                      productVariants.map((v, idx) => {
                                        const priceNum = parseFloat(v.price?.toString() || "0");
                                        const salePriceNum = parseFloat(v.sale_price?.toString() || "0");
                                        const discountPercent = priceNum && salePriceNum && priceNum > salePriceNum
                                          ...
                                          <tr key={idx} className="hover:bg-gray-50/50">
                                            ...
                                            <td className="px-6 py-3">
                                              <input
                                                ...
                                                className="w-28 bg-gray-50 border-2 border-black rounded-lg px-3 py-1.5 text-xs font-bold focus:outline-none"
                                                value={v.price}
                                                onChange={(e) => updateVariantField(idx, "price", parseFloat(e.target.value || "0"))}
                                                required
                                              />
                                            </td>
                                            <td className="px-6 py-3">
                    ...
                    <div className="pt-6 border-t border-black/5 flex gap-4">
                      ...
                      >
                        {productView === "add" ? "Tạo sản phẩm mới" : "Cập nhật sản phẩm"}
                      </button>
                    </div>
                  </form>
                </div>
          ...
          {activeTab === "orders" && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-serif font-black uppercase">Quản lý Đơn hàng</h2>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y-2 divide-black/5">
                      {orders.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="px-8 py-8 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">Không có đơn hàng nào chứa sản phẩm của shop bạn</td>
                        </tr>
                      ) : (
                        orders.map((order, i) => {
                          // Tính tổng giá trị của những mặt hàng thuộc shop trong đơn hàng này
                          let shopTotal = 0;
                          order.items.forEach((item: any) => {
                            shopTotal += parseFloat(item.total_price || 0);
                          });

                          return (
                            <tr key={order.id || i} className="hover:bg-gray-50 transition-colors group">
                              <td className="px-8 py-4 text-xs font-black uppercase text-gray-700">#{order.order_code}</td>
                              <td className="px-8 py-4 text-xs font-bold">
                                <p>{order.recipient_name || order.user?.profile?.full_name}</p>
                                <p className="text-[10px] text-gray-400">{order.recipient_phone || order.user?.phone}</p>
                              </td>
                              <td className="px-8 py-4 text-xs">
                                {order.items.map((item: any, idx: number) => (
                                  <div key={idx} className="mb-1 text-gray-600 font-medium">
                                    {item.product_name} <span className="font-black text-black">({item.size || item.variant?.size}/{item.color || item.variant?.color})</span> x{item.quantity}
                                  </div>
                                ))}
                              </td>
                              <td className="px-8 py-4 text-xs font-black text-primary">{shopTotal.toLocaleString()}₫</td>
                              <td className="px-8 py-4">
                                <span className={`text-[9px] font-black uppercase px-2 py-1 rounded border ${order.status === 'PENDING' ? 'bg-orange-50 text-orange-600 border-orange-100' : order.status === 'CONFIRMED' || order.status === 'PREPARING' ? 'bg-blue-50 text-blue-600 border-blue-100' : order.status === 'DELIVERED' || order.status === 'COMPLETED' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                                  {order.status === 'PENDING' ? 'Đang chờ' : order.status === 'CONFIRMED' ? 'Đã xác nhận' : order.status === 'PREPARING' ? 'Đang chuẩn bị' : order.status === 'DELIVERED' || order.status === 'COMPLETED' ? 'Hoàn tất' : 'Hủy bỏ'}
                                </span>
                              </td>
                              <td className="px-8 py-4 text-right">
          ...
          {activeTab === "promos" && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => {
                  setVoucherForm({
                    code: "", description: "", discount_type: "PERCENTAGE", discount_value: 0, min_order_amount: 0, max_discount: "", usage_limit: "", per_user_limit: 1, start_date: "", end_date: ""
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {vouchers.length === 0 ? (
                  <div className="col-span-3 text-center py-10 bg-white border-2 border-dashed border-black/10 rounded-2xl">
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Chưa có mã giảm giá nào được tạo</p>
                  </div>
                ) : (
                  vouchers.map((v, i) => (
                    <div key={v.id || i} className="bg-white border-2 border-black rounded-2xl p-6 shadow-sm relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-4 mb-6 relative z-10">
                        ...
                        <div>
                          <h4 className="text-lg font-black uppercase tracking-tight">
                            {v.discount_type === 'PERCENTAGE' ? `Giảm ${v.discount_value}%` : `Giảm ${v.discount_value.toLocaleString()}₫`}
                          </h4>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Mã: <span className="text-primary">{v.code}</span></p>
                        </div>
                      </div>
                      <div className="space-y-3 relative z-10">
                        <div className="flex justify-between text-xs">
                          <span className="text-gray-500 font-medium">Đã sử dụng</span>
                          <span className="font-black">{v.used_count || 0} / {v.usage_limit || "∞"}</span>
                        </div>
                        <div className="pt-4 flex justify-between items-center border-t border-gray-100">
                          <span className="text-[10px] font-bold text-gray-400">Đơn tối thiểu: {parseFloat(v.min_order_amount).toLocaleString()}₫</span>
                          <button onClick={() => handleDeleteVoucher(v.id)} className="text-[9px] font-black text-red-600 hover:underline uppercase">Xóa mã</button>
                        </div>
                      </div>
                    </div>
                  ))
                ...
                <div className="space-y-4">
                  {reviews.length === 0 ? (
                    <div className="text-center py-10 bg-white border-2 border-dashed border-black/10 rounded-2xl">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Chưa có bình luận hay đánh giá nào</p>
                    </div>
                  ) : (
                    reviews.map((rev) => (
                      <div key={rev.id} className="bg-white border-2 border-black rounded-2xl p-6 shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-black uppercase">
                              {rev.user?.profile?.full_name ? rev.user.profile.full_name.substring(0, 2) : "KH"}
                            </div>
                            <div>
                              <p className="text-xs font-black uppercase">{rev.user?.profile?.full_name || rev.user?.email || "Khách hàng"}</p>
                              <div className="flex gap-0.5 mt-0.5">
                                {Array.from({ length: 5 }).map((_, s) => (
                                  <Star
                                    key={s}
                                    size={10}
                                    className={s < (rev.rating || 5) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}
                                  />
                                ))}
                              </div>
                              {rev.product && (
                                <p className="text-[9px] text-gray-400 mt-1 uppercase font-bold">
                                  Sản phẩm: {rev.product.name} {rev.variant ? `(${rev.variant.size}/${rev.variant.color})` : ''}
                                </p>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="space-y-6">
                    {products.length === 0 ? (
                      ...
                      products.slice(0, 4).map((prod, idx) => (
                        <div key={prod.id || idx} className="flex items-center justify-between group">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-14 bg-gray-100 rounded-lg overflow-hidden border border-black/5 shrink-0">
                              <img src={prod.images?.[0]?.image_url || `https://images.unsplash.com/photo-1591047139829-d91aecb6caea?q=80&w=100`} className="w-full h-full object-cover" />
                            </div>
                            <div>
                              ...
                              <p className="text-[10px] font-bold text-gray-400 mt-1 flex items-center gap-1">
                                <Heart size={10} className="text-red-500 fill-red-500" /> {((prod.id * 17) % 150) + 12} lượt lưu
                              </p>
                            </div>
                          </div>
                          <span className="text-xs font-black">{prod.price.toLocaleString()}₫</span>
                        </div>
                      ))
                    )}
                  </div>
            ...
            <div className="space-y-10 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white border-2 border-black rounded-3xl p-8 shadow-sm">
                  ...
                  <h4 className="text-3xl font-black tracking-tighter text-black mt-2">{(stats.revenue || 0).toLocaleString()}₫</h4>
                </div>

                {/* Card 2: Doanh thu thực nhận */}
                <div className="bg-white border-2 border-black rounded-3xl p-8 shadow-sm">
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Doanh thu thực nhận (Sau khấu trừ 10%)</span>
                  <h4 className="text-3xl font-black tracking-tighter text-green-600 mt-2">{(stats.netRevenue || 0).toLocaleString()}₫</h4>
                </div>
                ...
                >
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 group-hover:text-primary transition-colors">Số dư khả dụng (Click điền nhanh)</span>
                    <h4 className="text-3xl font-black tracking-tighter text-primary mt-2">{(stats.availableBalance || 0).toLocaleString()}₫</h4>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-white border-2 border-black rounded-[2rem] p-8 shadow-sm">
                  <h3 className="text-xl font-black uppercase tracking-tighter mb-6">Yêu cầu rút tiền</h3>
                  <form onSubmit={handleWithdrawSubmit} className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Số tiền rút (VND)</label>
                      <input
                        type="number"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Ngân hàng thụ hưởng</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Tên chủ tài khoản (Không dấu)</label>
                      <input
                        type="text"
                    ...
                    <button type="submit" className="w-full py-4 bg-black text-white border-2 border-black rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary transition-all shadow-subtle active:translate-x-[1px] active:translate-y-[1px]">
                      GỬI YÊU CẦU RÚT TIỀN
                    </button>
                  </form>
                </div>
                ...
                <div className="lg:col-span-2 bg-white border-2 border-black rounded-[2rem] p-8 shadow-sm overflow-hidden">
                  <h3 className="text-xl font-black uppercase tracking-tighter mb-6 pb-2 border-b-2 border-black/5">Lịch sử rút tiền</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b-2 border-black/5 text-[10px] font-black uppercase tracking-widest text-gray-400 bg-gray-50/30">
                      ...
                      <tbody className="divide-y-2 divide-black/5 text-xs font-bold">
                        {withdrawals.length === 0 ? (
                          <tr>
                            <td colSpan={4} className="px-6 py-8 text-center text-gray-400 uppercase tracking-widest text-[10px]">Chưa có giao dịch rút tiền nào</td>
                          </tr>
                        ) : (
                          withdrawals.map((w, idx) => (
                            <tr key={w.id || idx} className="hover:bg-gray-50">
                              <td className="px-6 py-4 text-[10px] text-gray-500">{w.created_at ? new Date(w.created_at).toLocaleString("vi-VN") : "Gần đây"}</td>
                              <td className="px-6 py-4 font-black text-red-600">-{parseFloat(w.amount).toLocaleString()}₫</td>
                              <td className="px-6 py-4">
                                <p className="uppercase">{w.account_name}</p>
                                <p className="text-[10px] text-gray-400 font-medium">{w.bank_name} | {w.account_number}</p>
                              </td>
                              <td className="px-6 py-4">
                                <span className={`text-[8px] font-black uppercase px-2.5 py-1 rounded border ${w.status === 'PENDING' ? 'bg-orange-50 text-orange-600 border-orange-100' : w.status === 'APPROVED' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
          ...
          {activeTab === "settings" && (
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-serif font-black uppercase">Thông tin cửa hàng</h2>
              </div>
              ...
              <div className="bg-white border-2 border-black rounded-[2.5rem] p-10 shadow-sm max-w-3xl">
                <form onSubmit={handleSettingsSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Địa chỉ lấy hàng</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Mô tả shop</label>
                    <textarea
                      rows={3}
                      className="w-full bg-gray-50 border-2 border-black rounded-xl px-5 py-3.5 font-bold focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all resize-none"
                  ...
                  <button type="submit" className="px-8 py-4 bg-black text-white border-2 border-black rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary transition-all shadow-subtle active:translate-y-0.5 active:shadow-none">
          ...
          {activeTab === 'products' && productsTotal > 0 && (
            <div className="flex justify-center items-center gap-2 pt-12">
              ...
              {Array.from({ length: Math.ceil(productsTotal / 5) }).map((_, idx) => (
                <button
                  key={idx}
              ...
              <button
                onClick={() => setProductsPage(p => Math.min(p + 1, Math.ceil(productsTotal / 5)))}
                className="w-10 h-10 border-2 border-black rounded-xl flex items-center justify-center hover:bg-black hover:text-white transition-all active:translate-y-1"
              >
      ...
      {showVoucherModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white border-4 border-black rounded-[2.5rem] w-full max-w-2xl shadow-brutal p-8 relative">
            ...
            <h3 className="text-2xl font-serif font-black uppercase tracking-tighter mb-6">Tạo mã giảm giá mới</h3>
            <form onSubmit={handleVoucherSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Mã Coupon (Ví dụ: SALE20)</label>
                  <input
                    type="text"
                ...
                <CustomSelect
                  ...
                  onChange={(val) => setVoucherForm({ ...voucherForm, discount_type: val as "PERCENTAGE" | "FIXED_AMOUNT" })}
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giá trị giảm</label>
                  <input
                    type="number"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Đơn hàng tối thiểu (₫)</label>
                  <input
                    type="number"
                    className="w-full bg-gray-50 border-2 border-black rounded-xl px-4 py-3 font-bold"
                    value={voucherForm.min_order_amount}
                    onChange={(e) => setVoucherForm({ ...voucherForm, min_order_amount: parseFloat(e.target.value || "0") })}
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giảm tối đa (₫)</label>
                  <input
                    type="number"
                    className="w-full bg-gray-50 border-2 border-black rounded-xl px-4 py-3 font-bold"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Tổng lượt dùng tối đa</label>
                  <input
                    type="number"
                    className="w-full bg-gray-50 border-2 border-black rounded-xl px-4 py-3 font-bold"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Giới hạn/Người dùng</label>
                  <input
                    type="number"
                    className="w-full bg-gray-50 border-2 border-black rounded-xl px-4 py-3 font-bold"
                    value={voucherForm.per_user_limit}
                    onChange={(e) => setVoucherForm({ ...voucherForm, per_user_limit: parseInt(e.target.value || "1") })}
                    required
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Mô tả mã giảm giá</label>
                <input
                  type="text"
                  className="w-full bg-gray-50 border-2 border-black rounded-xl px-4 py-3 font-bold"
              ...
              <button type="submit" className="w-full btn-brutal py-4 text-xs font-black uppercase tracking-widest shadow-brutal hover:bg-primary active:translate-y-0.5 active:shadow-none">
                Tạo Voucher ngay
              </button>
            </form>
          </div>
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white border-4 border-black rounded-[2rem] w-full max-w-md shadow-brutal p-8 relative animate-in fade-in zoom-in-95 duration-150">
            ...
            <h3 className="text-xl font-serif font-black uppercase tracking-tighter mb-4 text-black">{confirmModal.title}</h3>
            <p className="text-gray-600 text-sm font-bold leading-relaxed mb-8">{confirmModal.message}</p>
