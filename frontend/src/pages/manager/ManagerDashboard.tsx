...
const ManagerDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { id: "vendor_approval", label: "Phê duyệt gian hàng", icon: <Store size={20} /> },
    ...
    <div className="min-h-screen bg-[#F4F4F0] flex overflow-hidden">
      ...
      <main className="flex-grow flex flex-col h-screen overflow-y-auto">
        ...
        <div className="p-10 max-w-7xl w-full mx-auto">
            ...
            <div className="space-y-10">
              ...
              {isStatsLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {[
                    { label: "Tổng doanh thu sàn", value: `${(stats?.totalRevenue ?? 0).toLocaleString()}₫`, sub: "Hoa hồng", icon: <BarChart3 className="text-blue-500" /> },
                    { label: "Tổng đơn hàng", value: stats?.totalOrders ?? 0, sub: "Toàn sàn", icon: <Box className="text-purple-500" /> },
                    { label: "Gian hàng chờ duyệt", value: stats?.pendingShops ?? 0, sub: "Cần duyệt", icon: <Store className="text-amber-500" /> },
                    { label: "Sản phẩm chờ duyệt", value: stats?.pendingProducts ?? 0, sub: "Cần kiểm duyệt", icon: <Box className="text-primary" /> },
                    { label: "Khiếu nại & Hoàn tiền", value: stats?.pendingDisputes ?? 0, sub: "Cần hòa giải", icon: <AlertCircle className="text-red-500" /> },
          ...
          {/* TAB: VENDOR APPROVAL */}
          {activeTab === "vendor_approval" && (
            <VendorTab
              showToast={(msg, type) => {
                addNotification(`${type === 'error' ? 'Lỗi: ' : ''}${msg}`);
              }}
          ...
          {activeTab === "moderation" && (
            <div className="space-y-8">
              ...
              {/* Header actions: Sub-tabs & Search */}
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="relative w-full md:w-72">
                  ...
                  <input
                    ...
                    className="w-full bg-white border-2 border-black/20 focus:border-black rounded-xl pl-12 pr-4 py-3 text-xs font-bold transition-all outline-none"
                  />
                </div>
              </div>
              ...
              {(isPendingProductsLoading || isActiveProductsLoading) ? (
                ...
                <div className="grid grid-cols-1 gap-6">
                  ...
                  {productSubTab === "pending" && (
                    <>
                      {!pendingProducts || pendingProducts.length === 0 ? (
                        ...
                        pendingProducts
                          .filter((p: any) => (p.name?.toLowerCase() || "").includes(productSearchTerm.toLowerCase()) || (p.shop?.shop_name?.toLowerCase() || "").includes(productSearchTerm.toLowerCase()))
                          .map((product: any) => {
                  ...
                  {productSubTab === "active" && (
                    <>
                      {(activeProducts?.filter((p: any) => p.approval_status === "APPROVED")?.length || 0) === 0 ? (
                  ...
                  {productSubTab === "blocked" && (
                    <>
                      {(activeProducts?.filter((p: any) => p.approval_status === "HIDDEN" || p.approval_status === "REJECTED")?.length || 0) === 0 ? (
                        <div className="bg-white border-2 border-black rounded-[2.5rem] p-16 flex flex-col items-center text-center">
                          <CheckCircle2 className="text-green-500 mb-4" size={48} />
                ...
                <div className="bg-white border-2 border-black rounded-[2rem] p-8 shadow-sm flex flex-col gap-6">
                  ...
                  <form onSubmit={handleCreateVoucher} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black uppercase tracking-wider block mb-2">Danh mục áp dụng</label>
                        <select
                          value={newVoucher.category_id}
                          onChange={(e) => setNewVoucher({ ...newVoucher, category_id: e.target.value })}
                          className="w-full bg-gray-50 border-2 border-black rounded-xl p-3 text-xs font-bold focus:outline-none"
                        >
                          <option value="">-- Toàn sàn --</option>
                                ...
                                <div className="flex-grow">
                                  ...
                                  <div className="text-[10px] font-bold text-gray-400 mt-1 space-y-0.5">
                                    ...
                                    <p>
                                      ...
                                      {" | "} Danh mục: <span className="text-black font-extrabold">{v.category?.name || "Toàn sàn"}</span>
                                    </p>
